...
async function loginAndTestReportes() {
  try {
    ...
    // Hacer login
    const loginResponse = await fetch('http://localhost:3001/api/v1/auth/verify', {
      method: 'POST',
      headers: {
    ...
    const reportesResponse = await fetch('http://localhost:3001/api/v1/reportes/estadisticas', {
      method: 'GET',
      headers: {
