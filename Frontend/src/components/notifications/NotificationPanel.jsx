...
const NotificationPanel = ({ isOpen, onClose }) => {
  ...
  // Preferencia del usuario (estado)
  ...
  // Escuchar cambios globales en preferencias
  useEffect(() => {
    const handler = () => {
      try {
        const saved = localStorage.getItem('notifications');
        setNotifEnabled(saved ? !!JSON.parse(saved).reservas : true);
      } catch (_) {}
    };
  ...
  // Marcar todas como leídas una sola vez al abrir el panel cuando están habilitadas
  useEffect(() => {
    if (!isOpen || !notifEnabled) return;
  ...
  }, [isOpen, notifEnabled, notifications]);

  // Debug (opcional)
  useEffect(() => {
    // console.log('NotificationPanel', { isOpen, notifEnabled, notifications, loading, error });
  }, [isOpen, notifEnabled, notifications, loading, error]);
  ...
  return (
    <>
      ...
      {/* Panel */}
      <div className="fixed right-0 top-0 h-full w-full max-w-md bg-white dark:bg-gray-800 shadow-xl z-50 transform transition-transform duration-300 ease-in-out flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 p-4">
          <div className="flex items-center gap-2">
            <Bell className="w-5 h-5 text-blue-600 dark:text-blue-400" />
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">{getPanelTitle()}</h2>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {!notifEnabled ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-600 dark:text-gray-400">Has desactivado las notificaciones de reservas en Configuración.</p>
            </div>
          ) : loading ? (
            ...
            <div className="text-center py-8">
              ...
              <button onClick={actions.refresh} className="mt-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
                Reintentar
              </button>
            </div>
            ...
            <div className="text-center py-8">
              <Bell className="w-12 h-12 text-gray-400 mx-auto mb-2" />
              <p className="text-gray-600 dark:text-gray-400">No hay notificaciones disponibles</p>
            </div>
          ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        actions.removeNotification(notification.id || (notification.data && notification.data._id));
                      }}
                    >
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-600 dark:text-gray-300 mb-2">{notification.message}</p>

                        <div className="flex items-center gap-2 mb-1">
                          <User className="w-4 h-4 text-gray-500" />
                          <span className="text-sm text-gray-600 dark:text-gray-300 truncate">
                            {autorNombre}
                            {autorDocumento ? ` (${autorDocumento})` : autorRol ? ` (Rol: ${autorRol})` : ''}
                          </span>
                        </div>
                        ...
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="text-xs text-gray-500 dark:text-gray-400">{formatDate(reserva.fecha || reserva.startDate)}</span>
                        </div>

                        <div className="flex items-center gap-2 mb-2">
                          <Clock className="w-4 h-4 text-gray-500" />
                          <span className="text-xs text-gray-500 dark:text-gray-400">{formatTime(notification.timestamp || reserva.createdAt || reserva.fechaCreacion)}</span>
                        </div>

                        <div className="flex items-center gap-2">
                          <span className={cn('text-xs font-medium px-2 py-1 rounded-full', getStatusColor(reserva.estado || reserva.status), 'bg-gray-100 dark:bg-gray-600')}>
                            {estadoLabel}
                          </span>
                        </div>

                        {reserva.proposito && (
                          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 truncate">{reserva.proposito}</p>
                        )}
                      </div>
                    </div>
                  </div>
        ...
        <div className="border-t border-gray-200 dark:border-gray-700 p-4 space-y-2">
          ...
          {notifications.length > 0 && (
            <button onClick={actions.clearAll} className="w-full px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors text-sm font-medium">
              Limpiar todas
            </button>
          )}
        </div>
